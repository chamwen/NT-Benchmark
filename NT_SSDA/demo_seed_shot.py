...
from utils.utils import lr_scheduler, fix_random_seed, op_copy, cal_acc_noimg, get_idx_ssda_seed
...
def data_load(X, y, args):
    ...
    sample_idx_te = tr.from_numpy(np.arange(len(idx_test))).long()
    data_tar_te = Data.TensorDataset(X[idx_test, :], y[idx_test], sample_idx_te)

    dset_loaders["target_te"] = Data.DataLoader(data_tar_te, batch_size=train_bs, shuffle=True)
...
if __name__ == '__main__':
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = '6'  # str(data_idx)
